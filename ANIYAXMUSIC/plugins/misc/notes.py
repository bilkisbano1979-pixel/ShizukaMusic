...
from ANIYAXMUSIC.utils.Aniya_ban import admin_filter
from ANIYAXMUSIC.mongo.notesdb import *
from ANIYAXMUSIC.utils.notes_func import GetNoteMessage, exceNoteMessageSender, privateNote_and_admin_checker
from ANIYAXMUSIC.utils.yumidb import user_admin
...
async def PrivateNote(client, message):
    ...
    if len(message.command) >= 2:
        ...
        if await is_pnote_on(chat_id):
            await message.reply(
                "Your notes are currently being sent in private. ANIYAXMUSIC will send a small note with a button which redirects to a private chat.",
                quote=True
            )
        else:
            await message.reply(
